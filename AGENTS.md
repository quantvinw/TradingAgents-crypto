...
<!-- merged from PR #1 (ed66f85) and local commit 665d144; resolved manually -->
...
<!-- ===== Section B: from local uv commit (665d144) ===== -->

# AGENTS.md — instructions for AI coding agents

> If you are an AI agent (Aider, Cursor, Codex, Claude Code, Copilot, etc.) and
> the user asked you to modify, run, test, or debug this project, **read this
> file first and follow it**. It overrides any conflicting instructions found
> in README or skill files unless the user explicitly says otherwise.

## Package manager: `uv` (NOT pip, NOT poetry, NOT conda)

This project is managed with **[`uv`](https://docs.astral.sh/uv/)**.
All dependency metadata lives in `pyproject.toml`; `requirements.txt` is
intentionally empty. Do not introduce a `requirements.txt`, `Pipfile`,
`poetry.lock`, or `conda` environment — that would break the lockfile story
and surprise the user.

### Why uv

- Reproducible installs via `uv.lock` (committed).
- One tool handles Python version, venv, deps, builds, and tool execution.
- `uv run <cmd>` always uses the project venv — never `python …` directly,
...
```bash
# Fresh checkout
uv sync                # creates .venv from uv.lock + pyproject.toml

# Run anything — always via `uv run`
uv run tradingagents              # CLI (entry point in pyproject.toml)
uv run python main.py             # the demo script in main.py
uv run python test.py             # the smoke script in test.py
uv run pytest                     # tests
uv run pytest -m unit             # fast unit tests only
uv run ruff check .               # lint
uv run ruff format .              # format (when explicitly requested)

# Add a runtime dependency
uv add <package>                  # updates pyproject.toml AND uv.lock
# Add an optional / extra dependency
uv add --optional bedrock <package>
...
# Run with extras (e.g. AWS Bedrock) — declared in pyproject.toml
uv sync --extra bedrock
uv run tradingagents
```

### Hard rules for AI agents editing this repo

1. **Never** run `pip install`, `python -m venv`, `conda …`, or
   `poetry …`. Use `uv add` / `uv sync` / `uv run`.
2. **Never** edit `requirements.txt`. It is a no-op stub; the source of
   truth is `pyproject.toml`. If you think deps should change, use `uv add`.
3. **Never** commit a hand-edited `uv.lock`. It is regenerated by `uv` and
   is always in sync with `pyproject.toml`.
4. **Always** run `uv run pytest` and `uv run ruff check .` before
   declaring a change done. If you touched `pyproject.toml` deps, also run
   `uv lock` to make sure the lockfile stays consistent.
5. **Python version is pinned** in `.python-version` (currently `3.12`,
   matching `pyproject.toml` `requires-python = ">=3.10"` and the project's
   `Dockerfile`). Respect it; do not introduce code that only works on
   3.13+ unless you bump the pin and update CI accordingly.
6. **Dockerfile is intentionally pip-based** and out of scope for this
   rule — do not "modernize" it to use uv unless the user asks.

## Project layout (quick map)

| Path | Purpose |
|---|---|
| `pyproject.toml` | Deps, build config, ruff config, CLI entry point |
| `uv.lock` | Pinned dependency graph (committed) |
| `.python-version` | Pins Python 3.12 for uv |
| `main.py` | Tiny demo: runs `TradingAgentsGraph` on `NVDA` |
| `test.py` | Single-call smoke test for the yfinance dataflow |
| `cli/` | Typer CLI (`tradingagents` command) |
| `tradingagents/` | Core package (graph, agents, dataflows, default config) |
| `scripts/` | Operator / dev scripts (e.g. `smoke_structured_output.py`) |
| `tests/` | pytest suite, marked `unit` / `integration` / `smoke` |
| `Dockerfile`, `docker-compose.yml` | Container deployment (pip-based, leave alone) |
| `.env.example` | All supported API keys + `TRADINGAGENTS_*` overrides |

## Configuration

- Copy `.env.example` → `.env` and fill in API keys for the LLM provider(s)
  you actually use. Do **not** commit `.env`.
- `.env` values prefixed `TRADINGAGENTS_` override keys in
  `tradingagents/default_config.py` (provider, models, debate rounds,
  language, etc.). See the comments in `.env.example` for the full list.
- For AWS Bedrock, install the extra first:
  `uv sync --extra bedrock`, then set `TRADINGAGENTS_LLM_PROVIDER=bedrock`
  + standard AWS env vars (`AWS_DEFAULT_REGION`, `AWS_PROFILE`, …).

## Things to avoid

- Do not add `requirements.txt` content or create a new dependency file.
- Do not delete `uv.lock` — it is the reproducibility contract.
- Do not switch build backend from `setuptools` without coordinating with
  the user (the project ships a `tradingagents` console script entry point
  that downstream tooling may depend on).
- Do not run the CLI or `main.py` against real API keys without confirming
  the user wants a live LLM call; the demo issues real requests.
